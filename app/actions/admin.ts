...
export async function addWatchlog(formData: FormData) {
  ...
  await supabase.from('watchlogs').insert({ title, src, type })
...
export async function deleteWatchlog(id: string) {
  await requireAdmin()
  const supabase = await createClient()
  await supabase.from('watchlogs').delete().eq('id', id)
  revalidatePath('/watchlogs')
  revalidatePath('/admin')
}
...
export async function addProject(formData: FormData) {
  ...
  await supabase.from('projects').insert({ title, description, image, link, tags })
...
export async function updateProject(formData: FormData) {
  ...
  await supabase.from('projects').update({ title, description, image, link, tags }).eq('id', id)
  revalidatePath('/projects')
  revalidatePath('/admin')
}
...
export async function updateWatchlog(formData: FormData) {
  ...
  await supabase.from('watchlogs').update({ title, src, type }).eq('id', id)
  revalidatePath('/watchlogs')
  revalidatePath('/admin')
}
