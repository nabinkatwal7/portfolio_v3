  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "PGRST116" || error.code === "PGRST205") {
        console.warn(
          "Guestbook table does not exist. Please run the database schema."
        );
        return [];
      }
      return [];
...
export async function getWatchlogs() {
  try {
    ...
    if (error) {
      ...
      if (error.code === "PGRST116" || error.code === "PGRST205") {
        console.warn(
          "Watchlogs table does not exist. Please run the database schema."
        );
        return [];
      }
      return [];
...
export async function getWatchlogsPaginated(
  page: number = 1,
  pageSize: number = 20
) {
  try {
...
export async function getProjects() {
  try {
    ...
    if (error) {
      ...
      if (error.code === "PGRST116" || error.code === "PGRST205") {
        console.warn(
          "Projects table does not exist. Please run the database schema."
        );
        return [];
      }
      return [];
...
export async function getBlogPosts() {
  try {
    ...
    if (error) {
      ...
      if (error.code === "PGRST116" || error.code === "PGRST205") {
        console.warn(
          "Blog posts table does not exist. Please run the database schema."
        );
        return [];
      }
      return [];
